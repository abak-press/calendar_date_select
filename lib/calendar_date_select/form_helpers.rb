...
module CalendarDateSelect::FormHelpers
  ...
  private
    ...
    def calendar_date_select_process_options(options)
      ...
      for key in [:default_time, :time, :format_24hr, :valid_date_check, :embedded, :buttons, :clear_button, :format, :year_range, :month_year, :popup, :hidden, :minute_interval] + callbacks
    ...
    def calendar_date_select_output(input, image, options = {}, javascript_options = {})
      ...
      if javascript_options[:embedded]
        ...
        out << javascript_tag("new CalendarDateSelect( #{CalendarDateSelect.jquery_noconflict}('#{uniq_id}').#{prev}, #{options_for_javascript(javascript_options)} ); ")
      else
        out << " "
        out << image_tag(image,
            :onclick => "new CalendarDateSelect( #{CalendarDateSelect.jquery_noconflict}(this).#{prev}, #{options_for_javascript(javascript_options)} );",
